...
{
    global $lang, $ajax;
    $result = strftime($format, $timestamp, $lang['__strftime_locale']);
    if ($ajax) {
        return $result; // already UTF-8, page is UTF-8
    }
    // Convert non-ASCII UTF-8 characters to HTML numeric entities
    // (works with any page charset, including windows-1252 and Czech chars)
    return preg_replace_callback('/[\x{0080}-\x{FFFF}]/u', function($m) {
...
function fix1252($s) {
global $ajax;
if ($ajax) return $s; // data is already UTF-8, no byte-level conversion needed
$replacement = array(
...
function tmdbLookupImdbId($title, $year = '', $originalTitle = '') {
    global $tmdb_api_key;
    $apiKey = (isset($tmdb_api_key) && $tmdb_api_key !== '') ? $tmdb_api_key : '134b15beef358903890a761f41dd98d8';
    ...
    if ($tmdbId === null && $originalTitle !== '' && $originalTitle !== $title) {
        $url2 = "https://api.themoviedb.org/3/search/movie?api_key=$apiKey"
              . "&query=" . urlencode($originalTitle)
              . "&include_adult=false";
        if ($year && $year != '0') $url2 .= "&year=$year";

        $response2 = @file_get_contents($url2, false, $ctx);
        if ($response2 !== false) {
            $data2 = @json_decode($response2, true);
            if (!empty($data2['results'])) {
                $tmdbId = $data2['results'][0]['id'];
            }
        }
    }
    ...
    $detailUrl = "https://api.themoviedb.org/3/movie/$tmdbId?api_key=$apiKey";
    $detailResponse = @file_get_contents($detailUrl, false, $ctx);
    if ($detailResponse === false) return null;
