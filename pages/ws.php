...
if (!empty($ajax)) {
    // AJAX mode: skip HTML shell and old menu scripts
    echo '<link rel="stylesheet" type="text/css" href="./format.css.php">';
    echo "<center><table width=\"100%\" cellpadding=0 cellspacing=0><tr class=f1><td colspan=$numcols>{$lang['WS']['HEADING']}</td></tr>";
    goto ws_content_start;
}
...
global $uclass, $ws_watcher, $usejpgraph, $dolast, $domost, $dobest, $doworst, $ignore_list, $WS_SELF, $ws_wb, $ColScheme;
    ...
    $uline = 0;
    $cmd = "SELECT ANY_VALUE(u.uid) AS uid,firstname,lastname FROM $DVD_TABLE d, $DVD_EVENTS_TABLE e, $DVD_USERS_TABLE u WHERE e.id=d.id AND e.uid=u.uid AND $ws_wb";
    if (($handleadult == 2) || (($handleadult == 1) && !$IsPrivate))
        $cmd .= " AND isadulttitle=0";
    ...
    while ($row = $db->sql_fetch_array($sql)) {
        ...
        if (!$ignore) {
            ...
            $tcnt = $row ? $row['count'] : 0;
            $trt = $row ? $row['runningtime'] : 0;
            $tavg = 0;
...
# Now get the months
    ...
    for ($mth=$maxmonths;$mth>0;$mth--) {
        ...
        $cmd = "SELECT SUM(runningtime) AS runningtime, SUM(1) AS count FROM $DVD_TABLE d, $DVD_EVENTS_TABLE e ";
...
global $db, $DVD_TABLE, $DVD_EVENTS_TABLE, $DVD_USERS_TABLE, $lang, $watched, $handleadult, $IsPrivate, $ws_wb, $ColScheme, $getimages, $img_webpath;
    ...
    $mouseline = 1;
    $cmd = "SELECT DISTINCT e.uid FROM $DVD_TABLE d, $DVD_EVENTS_TABLE e WHERE e.id=d.id AND e.id='$id' AND $ws_wb";
    if (($handleadult == 2) || (($handleadult == 1) && !$IsPrivate))
        $cmd .= " AND isadulttitle=0";
...
EOT;
        if ($dolast) {
            echo "<tr>";
            $cmd = "SELECT DISTINCT $DVD_EVENTS_TABLE.id, title, $DVD_EVENTS_TABLE.timestamp, reviewfilm FROM $DVD_EVENTS_TABLE LEFT JOIN $DVD_TABLE ON $DVD_EVENTS_TABLE.id=$DVD_TABLE.id WHERE $ws_wb";

            if (($handleadult == 2) || (($handleadult == 1) && !$IsPrivate))
                $cmd .= " AND isadulttitle=0";

            $cmd .= " ORDER BY $DVD_EVENTS_TABLE.timestamp DESC LIMIT $maxthumbs";
...
EOT;
        ...
        if ($domost) {
            echo "<tr>";
            $cmd = "SELECT $DVD_EVENTS_TABLE.id, SUM(1) AS count, ANY_VALUE(title) AS title, ANY_VALUE(reviewfilm) AS reviewfilm FROM $DVD_EVENTS_TABLE LEFT JOIN $DVD_TABLE ON $DVD_EVENTS_TABLE.id = $DVD_TABLE.id WHERE $ws_wb";
            if (($handleadult == 2) || (($handleadult == 1) && !$IsPrivate))
                $cmd .= " AND isadulttitle=0";

            $cmd .= " GROUP BY $DVD_EVENTS_TABLE.id ORDER BY count DESC, ANY_VALUE(sorttitle) LIMIT $maxthumbs";
...
EOT;
        ...
        if ($dobest) {
            echo "<tr>";
            $cmd = "SELECT DISTINCT $DVD_EVENTS_TABLE.id, title, reviewfilm, $DVD_EVENTS_TABLE.timestamp FROM $DVD_EVENTS_TABLE LEFT JOIN $DVD_TABLE ON $DVD_EVENTS_TABLE.id=$DVD_TABLE.id WHERE $ws_wb AND reviewfilm > '0'";
            if (($handleadult == 2) || (($handleadult == 1) && !$IsPrivate))
                $cmd .= " AND isadulttitle=0";

            $cmd .= " ORDER BY reviewfilm DESC, $DVD_EVENTS_TABLE.timestamp DESC LIMIT $maxthumbs";
...
EOT;
        if ($doworst) {
            ...
            $cmd .= " ORDER BY reviewfilm, $DVD_EVENTS_TABLE.timestamp DESC LIMIT $maxthumbs";
...
#
    ...
    if (!extension_loaded('gd')) {
        echo '<p>GD library is not available. Image generation requires the PHP GD extension.</p>';
        return;
    }
