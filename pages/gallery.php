...
if (!$ajax) header('Content-Type: text/html; charset="windows-1252";');
...
function css($divheight, $divwidth){
    global $Title, $imagewidth, $imageheight,$galleryver, $gallery_bgpic, $ajax;
    ...
    if ($ajax) {
        // AJAX mode: output only inline styles needed for gallery sizing
        echo "<style>\n";
        echo ".gallery { font-size:x-small; text-align:center; vertical-align:top; display:inline-block; width:{$divwidth}px; overflow:hidden; padding:3px; }\n";
        echo ".picbox { position:relative; padding:0; }\n";
        echo ".picbox img { width:{$imagewidth}px; {$imageheight} }\n";
        echo ".hide { display:none; } .show { display:block; }\n";
        echo ".plusbox { padding:3px; height:20px; width:20px; position:absolute; left:5px; top:5px; }\n";
        echo ".plusbox img { width:13px; height:13px; }\n";
        echo ".bigbox { padding:3px; height:20px; width:20px; position:absolute; right:5px; top:5px; }\n";
        echo ".print { display:none; } .noprint { display:block; }\n";
        echo "</style>\n";
        // dh function for box set toggle
        echo "<script>function dh(theitems,obj){var item=document.getElementById(theitems);if(item.className=='hide'){item.className='show';obj.src='gfx/minus.gif';}else{item.className='hide';obj.src='gfx/plus.gif';}}</script>\n";
        return;
    }
...
function site_header (){
    global $Title, $lang, $CurrentSiteTitle, $xmlfile, $sql, $dpp, $sortby, $order, $searchby, $searchtext, $letter, $ct, $page, $ajax;
    switch ($ct){
        case 'owned':
        case 'ordered':
        case 'wishlist':
    ...
    if ($ajax) {
        // AJAX mode: simplified header
        $pagination = generate_pagination(get_total_profiles($sql), $dpp, $dpp*($page-1), TRUE);
        echo "<div class=\"text-center mb-3\">\n";
        echo "<h4>$Title</h4>\n";
        if ($pagination) echo "<div>$pagination</div>\n";
        echo "</div>\n";
...
function site_footer(){
    global $sql, $dpp, $PHP_SELF,$_SERVER, $page, $ajax;

    if ($ajax) {
        echo "</div>\n"; // close gallery-container
        $pagination = generate_pagination(get_total_profiles($sql), $dpp, $dpp*($page-1), TRUE);
        if ($pagination) {
            echo "<div class=\"text-center mt-3\">$pagination</div>\n";
        }
        return;
    }
    ?>
